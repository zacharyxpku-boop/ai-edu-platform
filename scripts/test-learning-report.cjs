...
assert(profileJsSource.includes('homeSchoolConferenceKit') && profileWxmlSource.includes('家长今晚只看三件事'), 'Profile keeps conference-kit logic while tab UI stays parent-first and compact');
assert(profileJsSource.includes('familyDecisionIntakeSourcePlan') && profileWxmlSource.includes('材料缺口'), 'Profile keeps family input-lane logic and routes extra material through the current upload jump');
...
assert(profileJsSource.includes('aiLocalImplementationMatrix') && profileWxmlSource.includes('证据从哪来'), 'Profile keeps AI/local implementation logic while tab UI states the evidence boundary');
...
assert(profileWxmlSource.includes('parent-dash-route') && profileWxmlSource.includes('parent-dash-action-row'), 'Profile renders a compact route and action row instead of detailed report ledgers');assert(full.reportDraft.longTermPortrait.evidenceToCollect.length >= 3, 'long-term portrait asks for multiple evidence items, not a single data point');
