...
assert(profileJsSource.includes('crossWeekTrendBoard') && profileWxmlSource.includes('yd-parent-decision'), 'Profile keeps cross-week trend logic but exposes it through the compact report preview');
assert(profileJsSource.includes('homeSchoolCollaborationDigest') && profileWxmlSource.includes('判断依据'), 'Profile keeps home-school digest logic but exposes only a compact evidence source section on the tab page');
assert(profileJsSource.includes('homeSchoolConferenceKit') && profileWxmlSource.includes('今晚只看一个决策'), 'Profile keeps conference-kit logic while tab UI stays parent-first and compact');
...
assert(profileJsSource.includes('sourceEvidenceLedger') && profileWxmlSource.includes('不靠感觉下结论'), 'Profile keeps source evidence ledger logic and shows the evidence boundary without old ledgers');
assert(profileJsSource.includes('uploadedMaterialDecisionDossier') && profileWxmlSource.includes('今晚结论'), 'Profile keeps uploaded-material dossier logic behind the compact report preview');
...
assert(profileJsSource.includes('uploadedMaterialDecisionDossierMethodCandidateCards') && profileWxmlSource.includes('资料'), 'Profile keeps method candidate logic without rendering the old method ledger wall');
assert(profileJsSource.includes('uploadedMaterialDecisionDossierMethodologyGuide') && profileWxmlSource.includes('今晚结论'), 'Profile keeps methodology guide logic behind the compact report preview');
assert(profileJsSource.includes('uploadedMaterialDecisionDossierFamilyDecisionCalendar') && profileWxmlSource.includes('明天'), 'Profile keeps family decision calendar logic and exposes only the next review milestone');
...
assert(profileJsSource.includes('aiLocalImplementationMatrix') && profileWxmlSource.includes('判断依据'), 'Profile keeps AI/local implementation logic while tab UI states the evidence boundary');
...
assert(profileWxmlSource.includes('yd-parent-loop') && profileWxmlSource.includes('yd-parent-action-row'), 'Profile renders a compact route and action row instead of detailed report ledgers');
assert(profileWxmlSource.includes('yd-parent-source-row') && !profileWxmlSource.includes('parent-route-line'), 'Profile route is a visual evidence loop instead of a text-only line');
