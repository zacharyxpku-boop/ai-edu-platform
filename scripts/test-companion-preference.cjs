...
const legacyPreference = storage.loadCompanionPreference();
assert.strictEqual(legacyPreference.selectedCompanion, 'gudian', 'legacy six-teacher ids resolve to 咕点');
...
assert(homeWxml.includes('homeViewModel.teacherPickerLabel') && homeViewModelJs.includes('teacherPickerLabel'), 'home has a mascot entry through homeViewModel');
...
[
  ['home', homeWxml, '{{companionCopy.home}}', '{{homeViewModel.companionStrip}}'],
  ['review', reviewWxml, '{{companionCopy.review}}', '{{reviewViewModel.companionStrip}}'],
  ['tools', toolsWxml, '{{companionCopy.tools}}', '{{toolsViewModel.companionStrip}}'],
  ['profile', profileWxml, '{{companionCopy.profile}}', '{{profileViewModel.companionStrip}}']
...
assert(homeJs.includes('saveCompanionPreference'), 'home keeps legacy companion save compatibility');
assert(homeJs.includes('companionOptions'), 'home keeps legacy companion options data compatibility');
...
assert(teacherDoc.includes('孩子今天选择谁，谁就陪孩子走完整条 Tonight Route'), 'legacy teacher doc still documents prior companion behavior');
...
const tabPages = [homeWxml, homeViewModelJs, reviewWxml, reviewViewModelJs, toolsWxml, toolsViewModelJs, profileWxml].join('\n');
['帮我安排今晚学习', '先去说第一步'].forEach((cta) => {
  assert(tabPages.includes(cta), `main CTA remains: ${cta}`);
});
assert(reviewWxml.includes('{{reviewViewModel.primaryCta.text}}') && reviewViewModelJs.includes('开始 5 分钟修复'), 'review main CTA remains through reviewViewModel');
assert(tabPages.includes('{{profileViewModel.primaryCta}}') && profileViewModelJs.includes('完成今日复盘'), 'profile main CTA remains through profileViewModel');
