...
const methodologyModules = [
  ['目标启动模块', '动机型、目标感强但状态波动', '先明确为什么学、学到什么程度、如何判断完成', '今日任务 / 学习地图'],
...
function promptForPage(page, context = {}) {
  ...
  const promptBody = applyPromptContext(num, ({
    ...
    '24': ['一次报告不是终点，真正有价值的是持续更新的学情档案', ['报告结论进入学情档案，成为下次私教追问和复习任务的依据', 'AI私教负责高频追问，复习游戏验证记忆和迁移', '家长端看到证据变化、下一步任务和方法是否有效'], 'Long-term evidence loop: intake, report, tutor, review, parent evidence ledger, next update.']
  })[num] || [
