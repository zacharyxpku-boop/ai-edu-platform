...
function classifyHomework(text, weakPoints, minutes) {
  ...
  const items = lines.map((line, index) => {
    ...
    priority -= index * 1.5;
    const finalScore = Math.round(clamp(priority, 0, 100));
    return {
      id: `hw_${index + 1}`,
      text: line,
      score: finalScore,
...
function evidenceForLine(line, score, weakPoints, index) {
  ...
  if (/应用|综合|压轴|变式|方程|函数|几何|阅读/.test(line)) tags.push('迁移应用');
  ...
  const homeworkPlan = classifyHomework(payload.homeworkText || '', finalWeakPoints, payload.minutes || 35);
  return {
    tags,
    decision,
...
function buildAssessment(input) {
  ...
  return {
    ...
    weekly_review: buildWeeklyReview(axes, finalWeakPoints, homeworkPlan),
