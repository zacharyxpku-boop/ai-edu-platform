...
function routeForSearch(value) {
  ...
  if (/报告|分析|天赋|画像|导出/.test(text)) return 'report';
...
function rightRail() {
  ...
  const focus = {
    upload: ['资料完整度', '75%', '成绩、错题和测评已具备；补充老师反馈后置信度更高。', 'green'],
    report: ['报告可信度', '较高', '测评只做假设，成绩和错题负责交叉验证。', 'blue'],
    tutor: ['本题关联弱点', '多步计算顺序', '先让孩子说第一步，比直接算答案更重要。', 'orange'],
    review: ['今日挑战', '12 个知识点', '只奖励真实回忆和迁移，不鼓励机械刷题。', 'yellow'],
    parent: ['今晚只问一件事', '你的第一步怎么想？', '家长从催促者转为观察者和支持者。', 'green'],
    map: ['本周路径', '3 / 6', '当前停在 AI 点拨，完成后进入复习回访和家长复盘。', 'blue']
  }[state.active];
  if (!focus) {
    return `${progressCard}${evidenceCard}
  ...
  return `${progressCard}<article class="rail-card rail-focus-card ${focus[3]}"><b>${focus[0]}</b><strong>${focus[1]}</strong><p>${focus[2]}</p></article>${evidenceCard}`;
}
...
function renderHome() {
  return `
    ...
    <section class="hero-grid">
      <article class="buddy-message card"><div class="speech"><h1 class="mobile-hero-title">今晚先做哪一步？</h1><h2>晚上好，小明</h2><p>今天不用把所有信息摊开，只先找到最有效的一小步。</p><div class="chip-row"><span><i class="chip-dot"></i>连续学习 7 天</span><span>今晚 15-20 分钟</span></div></div><img class="mascot-img hero-avatar" src="${referenceAsset('hero-mascot.png')}" alt="咕点"></article>
...
function renderUpload() {
  const materialTypes = [
    ['天赋测评', '形成学习画像起点，不当作命运定论。', 'entry-report.png', 'green'],
    ['成绩单', '用真实表现验证学科优先级。', 'entry-upload.png', 'blue'],
    ['错题照片', '定位概念错、条件漏、模型错、计算错。', 'entry-review.png', 'orange'],
    ['学校反馈', '补充课堂状态和作业过程。', 'entry-parent.png', 'green'],
    ['家长观察', '记录动力、注意力、亲子沟通和作息。', 'hero-mascot.png', 'yellow']
  ];
  return `
...
function renderReport() {
  const evidence = [
    ['成绩单', '数学单元测验 82 分', '推理题得分高，计算题仍有失分。', 'green'],
    ['错题本', '条件漏读 12 道', '失分集中在审题、单位和步骤表达。', 'orange'],
    ['老师反馈', '课堂表达积极', '能主动提问，适合讲出来学。', 'green'],
    ['家长观察', '短任务稳定', '长任务需要可见进度和短反馈。', 'blue']
  ];
  return `
    <section class="page-title report-title"><div><h1>个性化报告</h1><p>先看证据，再看天赋、成绩与学习方法如何匹配。</p></div><button class="soft-button" data-action="print-report">下载 PDF 报告</button></section>
    ${sceneSwitch('report')}
    <section class="report-hero pro card"><article class="student-id-card"><div class="student-avatar"><span>小</span></div><div><h2>小明（四年级）</h2><p>原点智学样例档案</p><div class="report-meta"><span>学习时长 <b>4.2</b> 小时/天</span><span>报告周期 <b>7</b> 天</span><span>资料来源 <b>12</b> 项</span></div></div><img src="${referenceAsset('entry-report.png')}" alt="报告预览"></article><article class="ability-panel"><div class="card-head"><h3>能力雷达图</h3><span>本周水平</span></div><div class="radar-with-labels"><span>知识掌握</span><span>思维能力</span><span>学习习惯</span><span>学习动力</span><span>应用迁移</span><span>基础技能</span><div class="radar-shape strong"></div></div></article></section>
    <section class="evidence-band card"><div class="card-head"><h3>证据来源与置信度</h3><button class="soft-button mini" data-action="share-report">分享给家长</button></div><div class="evidence-band-grid">${evidence.map(([title, score, note, tone]) => `<article class="${tone}"><b>${title}</b><strong>${score}</strong><p>${note}</p></article>`).join('')}</div></section>
    <section class="confidence-board card"><h2>置信度边界</h2><div class="confidence-grid">${WEB_CONFIDENCE_BANDS.map(([title, source, rule]) => `<article><b>${title}</b><span>${source}</span><p>${rule}</p></article>`).join('')}</div></section>
    <section class="diagnostic-grid">${['听觉复述', '费曼学习法', '苏格拉底追问', '错因归因', '变式训练', '短周期反馈'].map((method, index) => `<article class="diagnostic-card card"><strong>${method}</strong><p>${index < 3 ? '匹配孩子当前优势，用输出验证理解。' : '补偿当前待开发点，用证据验证是否有效。'}</p><button class="soft-button mini" data-route="${index === 2 ? 'tutor' : 'review'}">去执行</button></article>`).join('')}</section>
    ${pageGuide('report')}
  `;
}
...
function renderTutor() {
  return `
    <section class="page-title"><div><h1>AI私教</h1><p>只追问第一步、依据和反例，不代写完整答案。</p></div><button class="soft-button" data-route="review">去复习岛</button></section>
    ${sceneSwitch('tutor')}
    <section class="tutor-lab"><article class="chat-card card"><div class="chat-head"><img src="${referenceAsset('entry-tutor.png')}" alt=""><div><h2>苏格拉底第一问</h2><p>先让孩子说出准备从哪里开始。</p></div></div><div class="chat-window"><div class="bubble coach">这道题问“一共”，你先准备把哪些数量放在一起？</div><div class="bubble me">我先算三种彩带一共有多少米。</div><div class="bubble coach">很好。那你会先写哪一个式子？为什么？</div></div><div class="chat-actions"><button class="soft-button" data-action="tutor-stuck">我有点卡住</button><button class="soft-button" data-action="tutor-hint">给我一点提示</button><button class="soft-button" data-action="tutor-retry">我想再试一次</button></div><div class="input-line"><input id="tutorInput" placeholder="告诉我你的想法..."><button data-action="send-tutor">发送</button></div><p class="tutor-boundary">边界：只做思路提示和追问，不提供整题代写。</p></article><article class="problem-board card"><div class="card-head"><h3>题目与思路板</h3><button class="soft-button mini" data-action="tutor-hint">提示</button></div><div class="problem-card"><span>应用题</span><p>活动场地买了红色 45 米、蓝色 27 米、黄色 18 米彩带。一共买了多少米？</p><b>先想：要求“一共”，要把哪些数量合并？</b></div><div class="board-and-ladder"><div class="thinking-canvas"><h3>我的思路</h3><div class="canvas-empty">写下你的步骤、图示或列式。</div></div><div class="hint-ladder"><h3>提示阶梯</h3><ol><li class="active"><b>第 1 步</b><p>先理解题目问什么。</p></li><li><b>第 2 步</b><p>找出需要合并的数量。</p></li><li><b>第 3 步</b><p>列式并检查结果。</p></li></ol></div></div></article></section>
    ${pageGuide('tutor')}
  `;
}
...
function renderReview() {
  const levels = [
    ...
    ['错因复盘', '把错误变成检查清单。', 'entry-tutor.png', 'orange', '奖励 25 星']
  ];
  return `
    ...
    ${sceneSwitch('review')}
    <section class="review-world card"><div class="world-path"><span></span></div><div class="world-node node-1 done"><img src="${referenceAsset('entry-report.png')}" alt=""><b>回忆关</b><em>已完成</em></div><div class="world-node node-2 done"><img src="${referenceAsset('entry-map.png')}" alt=""><b>迁移关</b><em>已完成</em></div><div class="world-node node-3 active"><img src="${referenceAsset('entry-review.png')}" alt=""><b>变式挑战</b><em>进行中</em></div><div class="world-node node-4 locked"><img src="${referenceAsset('entry-tutor.png')}" alt=""><b>错因复盘</b><em>待解锁</em></div><button class="world-rule-button" data-action="review-map-info">查看规则</button></section>
    ${pageGuide('review')}
